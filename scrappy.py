...
if __name__ == "__main__":
    ...
    scraper0 = WebScraper(
        "https://sites.google.com/view/arka-test/", delay=0, max_threads=6
    )
    
    scraper = WebScraper(
        "https://scyantra.com", delay=0, max_threads=6
    )
    ...
    # for k, v in output.items():
        # element = {
        #     "url": k,
        #     "content": v["content"][:10],
        #     "title": v["title"][:10],
        #     "embedding": randint(0, 10),
        #     "metadata": {"date": datetime.now()},
        #     "gid": group_id,
        # }
    print(results[0])
        # db_supposed_input.append(self.results)

    # pprint(db_supposed_input)

    # for k, v in output.items():
    #     counter = 0
    #     try:
    #         file_name = k.split("/")[-1] + ".md"
    #     except Exception:
    #         file_name = f"{counter}.md"
    #
    #     path = Path(__file__).parent.joinpath("lol", file_name)
    #     # path = f"lol/"
    #     print(path)
